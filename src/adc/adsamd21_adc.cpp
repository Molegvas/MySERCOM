...
typedef enum _mRef
{
  ...
  MR_INTVCC0,         // 1/1.6 VDDANA 1.48?
  MR_INTVCC1,         // 1/2 VDDANA
  MR_AREFA,           // External Reference
  MR_AREFB,           // External Reference
} mRef;
//=====================================================
...
int16_t analogDifferential(uint8_t pin_pos,uint8_t pin_neg) 
{
  ...
  if((g_APinDescription[pin_neg].ulADCChannelNumber>0x07) && (pin_neg<ADC_PIN_GND)) 
  { // if the negative pin is out of bounds
    return 0;
  }  
...
void analogRef( uint8_t ref )
{
  ...
  switch (ref)
  {
    ...
    case MR_INTVCC1:          // 1/2 VDDANA
      ADC->REFCTRL.bit.REFSEL = ADC_REFCTRL_REFSEL_INTVCC1_Val; // 1/2 VDDANA = 0.5* 3V3 = 1.65V
      break;

    case MR_AREFA:            // External Reference
      ADC->REFCTRL.bit.REFSEL = ADC_REFCTRL_REFSEL_AREFA_Val;
      break;

    case MR_AREFB:            // External Reference
